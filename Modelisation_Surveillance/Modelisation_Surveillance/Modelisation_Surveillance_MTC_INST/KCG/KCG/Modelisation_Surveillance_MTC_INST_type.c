...
int Result_MTC_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    ...
    return pConverter->m_bImportedAsHex ?
        pConverter->m_pfnImportedToString(pValue, sizeof(Result_MTC), pfnStrAppend, pStrObj) :
        pfnStrAppend("n/a", pStrObj)
    ;
}
...
int string_to_Result_MTC(const char *str, void *pValue, char **endptr)
{
    ...
    if (nRet == 0) {
        return pConverter->m_bImportedAsHex ?
            pConverter->m_pfnStringToImported(str, pValue, sizeof(Result_MTC), endptr) :
            0
        ;
    }
    return nRet;
...
int init_Result_MTC(void *pValue)
{
    Result_MTC_init((Result_MTC*)pValue);
    return 1;
}

int release_Result_MTC(void *pValue)
{
    if (pSimResult_MTCVTable != NULL
        && pSimResult_MTCVTable->m_version >= ScvR18
        && pSimResult_MTCVTable->m_pfnRelease != NULL) {
            return pSimResult_MTCVTable->m_pfnRelease(pValue);
    }
    return 1;
}

int copy_Result_MTC(void *pToValue, const void *pFromValue)
{
    *((Result_MTC*)pToValue) = *((Result_MTC*)pFromValue);
    return 1;
}
